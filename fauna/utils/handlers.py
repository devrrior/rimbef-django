...
from fauna.apps.archivos import exceptions as sensor_exceptions
...
def fauna_api_exception_handler(exc, context):
    ...
    if response is not None:
        ...
        if any(
            (
                type(exc) is sensor_exceptions.SupportFileDoesNotExistsAPIException,
                type(exc) is sensor_exceptions.SensorAlreadyExistsAPIException,
                type(exc) is sensor_exceptions.SensorDoesNotExistsAPIException,
            )
        ):
            logger.warning(exc)
